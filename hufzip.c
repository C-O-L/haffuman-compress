...
int main()
{
	while (1)
	{
		int flag = 0;		// 每次进入循环都要初始化flag为0
		char opt;
		char ifname[256] = { 0 }, ofname[256] = { 0 };		// 保存输入输出文件名
		// 输入所选择操作类型的数字代号：c：压缩，e：解压，q：退出
		...
		else
		{
			...
			//fflush(stdin);		//清理标准输入流，把多余的未被保存的数据丢掉,防止干扰gets函数读取文件名
			scanf("%*[^\n]");       //清理输入缓冲区中第一个\n之前的所有字符
		...
		case 'c': puts("Compressing……");
			flag = compress(ifname, ofname);	// 压缩，返回值用于判断文件名是否不存在
			break;
		case 'e': puts("Extracting……");
			flag = extract(ifname, ofname);		// 解压，返回值用于判断文件名是否不存在
			break;
		}
		if (flag == -1)
			puts("Sorry, infile \"%s\" doesn't exist!", ifname);		// 如果标志为‘-1’则输入文件不存在
		else
